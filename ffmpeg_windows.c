...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps)
{
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer), "ffmpeg.exe -loglevel verbose -y -f rawvideo -pix_fmt rgba -s %lldx%lld -r %lld -i - -c:v libx264 -preset veryslow -qp 0 -c:a aac -ab 200k -pix_fmt yuv420p output.mp4", width, height, fps);
